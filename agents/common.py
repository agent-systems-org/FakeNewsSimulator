    ...
    class AcceptNews(CyclicBehaviour):
        async def run(self):
            ...
            if not msg:
                ...
                #     self.agent.log(f"already received msg {content.id}")
                if (content not in self.agent.beliving) and (content not in self.agent.debunking):
                    # its math time
                    ...
                    if(result > 50 - (M - self.agent.disposition)):  # accept the msg
                        self.agent.disposition = self.agent.disposition + CONV_C * (1 - E)
    ...
    class ShareDebunk(PeriodicBehaviour):
        async def run(self):
            if self.agent.adj_list and self.agent.debunking:
                ...
                self.agent.log(
                        f"spreading debunk to {num_rand_recipients} recipients"
                        )
                msgs = []
                for recipient in rand_recipients:
                    msg = Message()
